...
- 同步吸收 Issue [#61](https://github.com/FoundZiGu/GuJumpgate/issues/61) 与 Issue [#127](https://github.com/FoundZiGu/GuJumpgate/issues/127) 的邮箱 provider 扩展需求：
...
- 邮箱 provider 扩展：
  - 新增 `MoeMail` 支持，对应 Issue [#61](https://github.com/FoundZiGu/GuJumpgate/issues/61)。
  - 新增 `YYDS Mail` 支持，对应 Issue [#127](https://github.com/FoundZiGu/GuJumpgate/issues/127)。
  - 两个 provider 都已接入注册邮箱生成、验证码轮询和主流程读取链路。
- Hotmail Helper 打包体验补齐：
  - Windows 发布包现在内置轻量 Python 运行时。
  - `start-hotmail-helper.bat` 会优先使用包内 `python\python.exe` 启动 `scripts\hotmail_helper.py`。
  - 大多数情况下不再需要用户额外手动安装 Python，即可直接启动本地 Hotmail Helper。
...
- WhatsApp 识别逻辑收敛：
  ...
  - 对 `Send code via WhatsApp SMS`、`通过以下方式发送验证码 短信 WhatsApp` 这类混合文案不再误判为必须重开 OAuth 的 WhatsApp 页面。
- Hotmail 管理器增强：
  - 新增从后台主动回读最新状态的同步逻辑。
